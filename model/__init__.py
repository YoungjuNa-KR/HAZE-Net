...
def dataparallel(model, gpu_list):
    ...
    assert isinstance(model, list), "Invalid Type of model"
...
class Model(nn.Module):
    def __init__(self, opt, ckp):
        ...
        self.model = HAZE_SRx2.make_model(opt).to(self.device)
    ...
    def save(self, path, is_best=False):
        ...
        torch.save(
            target.state_dict(), 
            os.path.join(path, 'model', 'gazemodule_latest.pt')
        )
        if is_best:
            torch.save(
                target.state_dict(),
                os.path.join(path, 'model', 'gazemodule_best.pt')
            )
